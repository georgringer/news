...
class ext_update
{
    ...
    protected function updateFileRelations()
    {
        ...
        if ($countFilesWithParent === 0 && $countNewsWithFileRelation > 0) {
            ...
            if ($countNewsWithFileRelation == 0) {
                $this->messageArray[] = [FlashMessage::NOTICE, $title, 'Nothing to do! There are no news with related files'];
            }
            if ($countFilesWithParent != 0) {
                $this->messageArray[] = [FlashMessage::NOTICE, $title, 'Can not update because there are files with new relation model!'];
            }
        }
    }
    ...
    protected function updateContentRelationToMm()
    {
        ...
        if ($countMmTable === 0 && $countContentElementRelation > 0) {
            ...
            $this->messageArray[] = [FlashMessage::NOTICE, $title, 'Not needed/possible anymore as the mm table is already filled!'];
        }
    }
    ...
    protected function updateParentFieldOfMigratedCategories(array $oldNewCategoryUidMapping)
    {
        ...
        foreach ($toUpdate as $row) {
            if (!empty($oldNewCategoryUidMapping[$row['parentcategory']])) {
                ...
                $this->databaseConnection->exec_UPDATEquery('sys_category', 'uid='.$sysCategoryUid, ['parent' => $newParentUId]);
                $updatedRecords++;
            }
        }
    ...
    protected function updateCategoryPermissionFields($table, array $oldNewCategoryUidMapping)
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($newCategoryPerms !== $row['category_perms']) {
                $this->databaseConnection->exec_UPDATEquery($table, 'uid='.$row['uid'], ['category_perms' => $newCategoryPerms]);
                $updatedRecords++;
            }
        }
