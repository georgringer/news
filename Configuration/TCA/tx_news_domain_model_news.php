...
if ($configuration->getUseFal()) {
    ...
    if ($configuration->getUseFal() === 1) {
        foreach ($tx_news_domain_model_news['types'] as $key => $config) {
            $tx_news_domain_model_news['types'][$key]['showitem'] = str_replace([',media,', ',related_files,'], [',fal_media,', ',fal_related_files,'], $config['showitem']);
        }
    ...
    // use FAL and media multimedia
        ...
        foreach ($tx_news_domain_model_news['types'] as $key => $config) {
            $tx_news_domain_model_news['types'][$key]['showitem'] = str_replace([',media,', ',related_files,'], [',fal_media,media,', ',fal_related_files,'], $config['showitem']);
        }
        unset($tx_news_domain_model_news['columns']['related_files']);
    ...
    // use both
        ...
        foreach ($tx_news_domain_model_news['types'] as $key => $config) {
            $tx_news_domain_model_news['types'][$key]['showitem'] = str_replace([',media,', ',related_files,'], [',fal_media,media,', ',fal_related_files,related_files,'], $config['showitem']);
        }
    }
}
