...
class Tx_News_Cache_ClassCacheParser {
...
//		print_r($cachedFileContent);die('-');
	...
	public static function parse($key) {
		...
		$classCacheService = t3lib_div::makeInstance('Tx_News_Cache_ClassCacheService');
		$classCacheService->setCache($GLOBALS['typo3CacheManager']->getCache('class_cache'));

		if (!$classCacheService->has($identifier)) {
			...
			$classCacheService->store($identifier, $code);
		}
		return $identifier;
	}
